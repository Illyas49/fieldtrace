            ...
            {steps.map((step, index) => {
              ...
              return (
                <div key={index} className="relative group">
                  <div className="bg-white p-8 rounded-xl border-2 border-gray-200 h-full shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 hover:border-mustard-400">
                    <div className="absolute -top-5 -left-5 w-12 h-12 bg-gradient-to-br from-mustard-600 to-mustard-700 text-white rounded-full flex items-center justify-center font-bold text-xl shadow-xl group-hover:scale-110 transition-transform">
                      {index + 1}
                    </div>
                    <div className="w-14 h-14 bg-mustard-100 rounded-xl flex items-center justify-center mb-5 mt-4 group-hover:bg-gradient-to-br group-hover:from-mustard-500 group-hover:to-mustard-600 transition-all">
                      <Icon size={26} className="text-mustard-600 group-hover:text-white transition-colors" />
                    </div>
                    <h3 className="text-lg font-bold text-gray-900 mb-4">{step.title}</h3>
                    <p className="text-gray-600 leading-relaxed text-sm">{step.description}</p>
                  </div>
                  {index < 3 && (
                    <div className="hidden lg:block absolute top-1/2 -right-4 w-8 h-0.5 bg-mustard-300 -translate-y-1/2 z-0">
                      <div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-2 bg-mustard-500 rounded-full"></div>
                    </div>
                  )}
                </div>
              );
